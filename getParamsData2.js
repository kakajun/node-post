 ...
 * @param {*} code  编码 BCG
 */
export function getParamsDl(index, code, name, road = 1, bm) {
  const voltageOrCurrent = name.includes('电流') ? '电流' : '电压';
  const bzcdName = `支路${voltageOrCurrent}${index}组串式`;
  function getbaseKeyword(cdModelName) { return `${bzcdName}_${cdModelName}_${code}_${bm}_${fc}` };
  // 初始化关键词数组
...
export function getParamsDlData(index, code, name, road = 1, arrear, bm) {
